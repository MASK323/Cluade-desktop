...
1. 先打开ClaudeDesktop-Installer.exe，安装最新版本的CluadeDesktop（这只是一种安装方法，你也可以用官方和其他的）。
...
8. 将网盘里的claude-code、claude-code-vm、vm_bundles拷贝到 `C:\Users\<用户名>\AppData\Local\Claude-3p` 目录中，请注意替换为你自己的用户名。
   管理员基本上都是这个路径 `C:\Users\Administrator\AppData\Local\Claude-3p`。
9. 启动coworkvm，打开claude desktop测试沙箱，没问题即可使用。

**备注：**
1. 遇到错误让codex查看 `C:\Users\<用户名>\AppData\Local\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Local\Claude-3p\logs` 中的日志。
...
### 排版技巧总结：
*   **命令与路径**：凡是涉及到具体的命令行指令（如 `Add-AppxPackage`）或文件路径（如 `C:\Users\...`），建议包裹在反引号 `` ` `` 中，使其显示为灰色背景框，既美观又能避免格式错乱。
*   **代码块隔离**：如果是一大段配置代码，请在开头使用三个反引号 ```` ``` ````，结尾同样使用三个反引号，这样就能保证完美的缩进和换行。
